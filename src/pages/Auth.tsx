...
type Portal = "passenger" | "driver" | "admin";

export default function Auth() {
  const [searchParams] = useSearchParams();
  const defaultPortal = (searchParams.get("portal") as Portal) || "passenger";
  ...
  const portalConfig = {
    passenger: { icon: <User className="w-8 h-8 text-primary" />, label: "Passenger", desc: "Track your bus in real time" },
    driver:    { icon: <Bus className="w-8 h-8 text-primary" />, label: "Driver", desc: "Start your trip and broadcast GPS" },
    admin:     { icon: <ShieldCheck className="w-8 h-8 text-primary" />, label: "Admin", desc: "Fleet control & management" },
  };
    ...
    <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center p-6 bg-[url('/grid.svg')] bg-center">
      ...
      >
        <div className="glass-card p-8 relative overflow-hidden backdrop-blur-xl border-white/5 shadow-2xl">
          ...
          {portal === "passenger" && !loading && (
            <div className="mt-6 text-center">
              <button
                onClick={() => setIsSignUp(!isSignUp)}
                className="text-sm text-primary/80 hover:text-primary transition-colors hover:underline"
              >
                {isSignUp ? "Already have an account? Sign in" : "New here? Create a passenger account"}
              </button>
            </div>
          )}
        </div>
